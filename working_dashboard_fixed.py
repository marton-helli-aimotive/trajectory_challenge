#!/usr/bin/env python3
"""
Fixed dashboard for E2E testing with proper element selectors.
"""
...
def main():
    ...
    # Sidebar navigation with proper IDs
...
def show_overview():
    ...
    with col1:
        ...
        if st.button("Go to Trajectory Visualization", key="go_to_trajectory_btn"):
    ...
    with col2:
        ...
        if st.button("Go to Model Comparison", key="go_to_comparison_btn"):
...
def show_trajectory_visualization():
    ...
    # Trajectory selection with proper key
    trajectory_idx = st.selectbox(
        "Select Trajectory",
        range(5),
        format_func=lambda x: f"Vehicle {x}",
        key="trajectory_selectbox"
    )
    ...
    with col2:
        ...
        if show_prediction:
            selected_models = st.multiselect(
                ...
                key="prediction_models_multiselect"
            )
...
def show_model_comparison():
    ...
    # Model selection with proper key
    selected_models = st.multiselect(
    ...
    # Run comparison
    if st.button("Run Model Comparison", key="run_comparison_btn"):
...
def show_model_explainability():
    ...
    selected_model = st.selectbox(
        ...
        ["Constant Velocity", "Constant Acceleration", "Polynomial Regression", "KNN", "Gaussian Process", "Ensemble"],
        key="explainability_model_selectbox"
    )
...
def show_settings():
    ...
    new_update_frequency = st.selectbox(
        ...
        key="settings_update_frequency_selectbox"
    )
    ...
    with col1:
        default_plot_type = st.selectbox(
            "Default Plot Type",
            ["2D Trajectory", "3D Trajectory", "Velocity Profile", "Acceleration Profile"],
            key="settings_plot_type_selectbox"
        )
    ...
    with col2:
        color_scheme = st.selectbox(
            ...
            key="settings_color_scheme_selectbox"
        )
