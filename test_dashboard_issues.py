#!/usr/bin/env python3
"""
Test script to identify and fix dashboard issues:
...
class DashboardIssueTester:
    ...
    def test_2d_trajectory_plot(self):
        ...
        try:
            ...
            # Test the problematic plot creation
            fig = go.Figure()
            ...
            fig.add_trace(go.Scatter(
                ...
                marker=dict(
                    size=6, 
                    color=v, 
                    colorscale='Viridis',  # This should be marker_colorscale
                    showscale=True
                )
            ))
    ...
    def test_ngsim_data_integration(self):
        ...
        try:
            # Import NGSIM dataset
            from vehicle_trajectory_prediction.data.datasets import NGSIMDataset, DatasetConfig, DatasetFactory
            ...
            if data_path.exists():
                ...
                self.log_test("NGSIM Dataset Creation", True, "NGSIM dataset created successfully")
                
                # Try to load data
                try:
                    data = dataset.load_data()
                    self.log_test("NGSIM Data Loading", True, f"Loaded {len(data)} rows of NGSIM data")
                except Exception as e:
                    self.log_test("NGSIM Data Loading", False, f"Error loading data: {str(e)}")
            else:
            ...
            self.log_test("NGSIM Import", False, f"Import error: {str(e)}")
        except Exception as e:
